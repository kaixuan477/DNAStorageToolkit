//access pre-existing system header files, not found in github repo
#include <bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp>

//accessing user program files under ../includes/...
...
#define DEBUG 0
//#define endl "\n"
#define all(v) v.begin(),v.end() //macro for reading all elements in vector

//Using GNU Policy Based Data Structures for efficient data handling
...
int w; //config file anchor_length
int l; //config file hash_minus_anchor_length
int r; //config file cluster_diameter
int num_local_iter;
int num_global_iter;
int theta_low;
int theta_high;
...
vector<int> wrongEle;
pthread_mutex_t mutex1; //mutex for managing concurrency
...
//Function to generate all possible q-gram sequences -
//recursion to create all q-length substrings from iterable {A, C, G, T} vector
void q_gram_seq_generator(int length, string curr){
  if(length == 0){
    ...
    for(int i = idx;i < iterable_size ; ++i){
      curr.push_back(iterable[i]); //string.push_back() to append char to end of string
      q_gram_seq_generator(length-1 , curr);
      curr.pop_back(); //string.pop_back() removes last char from string
    }
  }
}

//Function to take DNA sequence block to identify all present q-grams,
//stores result in temp bitset that references global q-gram dict (pool) by string s
inline void q_gram_string(const string & s , int offset , bitset<Q_GRAM_STRAND_SIZE> &temp_gram_string){
  ...
  unordered_set<string> block_grams; //set to store unique q-garms present in substring
  string gram;
  for(int i = 0; i < gram_size-1 ; ++i){ //gram contains gram-1 chars of string s
    gram.push_back(s[i]);
  }

  //builds set of all specified size q-grams
  for(int i = gram_size-1 , len = s.size() ; i < len ; ++i){ 
    gram.push_back(s[i]);
    block_grams.insert(gram); //inserts grame into set
    gram.erase(gram.begin()); //erases first element from string
  }
  if(DEBUG) assert(gram.size() == gram_size-1);

  //updates presence of each q-gram in block_grams against all_q_gram_sequences
  for(int i = 0, len = all_q_gram_sequences.size() ; i < len ; ++i){
    if(offset*len+i >= Q_GRAM_STRAND_SIZE){ //overflow check ensures bitset size < Q_GRAM_STRAND_SIZE
      cout << "Change the variable Q_GRAM_STRAND_SIZE to: " << 2*strand_length*all_q_gram_sequences.size()/block_size << endl;
      exit(EXIT_FAILURE);
    }
    //updates temp bitset to reflect presence of q-gram
    temp_gram_string[offset*len+i] = (block_grams.find(all_q_gram_sequences[i]) != block_grams.end()); 
  }
}

//Function to compute all possible q-grams for single DNA strand obtained from global pool,
//breaks DNA strand into blocks of fixed size and runs q_gram_string() on each,
//finally updates result to global q_gram_dict bitset
inline void q_gram_sequence(int idx){
  string s = pool[idx];
  bitset<Q_GRAM_STRAND_SIZE> temp_gram_string; //temp bitset to store q_gram_string() result
  
  //breaks DNA into blocks
  ...
  //residual bit handling for incomplete block
  if(j != 0){
...
//Function to process individual DNA strands in parallel,
//runs q_gram_sequence() for assigned range of indexes of pool within thread
void *q_gram_thread(void *args){ //args is type pair<int, int>
...
void create_q_gram_dict(){
  ...
  int extra = pool.size()%num_thread; //handling residual DNA strands
  pair<int,int> info_thread[num_thread];
  pthread_t my_thread[num_thread];
  int curr = 0;

  //Initializes and manages threads to process all DNA strands
  ...
  //joins parallelised results, results stored in global q_gram_dict and indexed
  for(int i = 0;i < num_thread ; ++i)
    pthread_join(my_thread[i] , NULL);
}

//Calculates the edit distance, 
//the minimum number of edits to transform s1 to s2
int edit_distance(const string &s1, const string &s2){
  int len1 = s1.length() , len2 = s2.length();
  //uses matrix to simulate tree structure, stores min edit distance for
  //each possible insert/delete/substitute match
  int dp[2][len1+1];
  ...
  //Iteratively to check char matches between s1 and s2 elements,
  //otherwise compute min of deleting, inserting or substituting char
  ...
  return dp[len2&1][len1]; //edit distance result
}

//Generate random string of specified length using iterable {A, C, G, T}
...
//Computes the Hamming distance, which approximates edit distance:
//counts the number of differing char in position
int hamming_distance(int i , int j){
  return (q_gram_dict[i]^q_gram_dict[j]).count(); //XOR operation reflects "1" when different, count "1"s
}

//Compute distance based on type (edit or hamming)
...
//Compute hash value for a given DNA strand using anchors matrix
inline string getHashValue(const string &s, const vector<vector<string> > &anchors){
  ...
  //iterate over sets of anchors for present anchors in DNA strand
  for(int i = 0 , rows = anchors.size() ; i < rows ; ++i){
    int pos = -1;
    for(int j = 0, cols = anchors[i].size() ; j < cols ; ++j){
      if(pos == -1)
        pos = s.substr(0 , len-w-l).find(anchors[i][j]); //exit on first anchor match
      else
        break;
    }
    if(pos == -1) continue;
    hash_value += s.substr(pos , w+l); //for each set, concatenate substring to hash value
  }
  return hash_value;
}

//Function to process single thread for hashing
void *global_hashing_thread(void *args){
  ...
  pthread_mutex_lock(&mutex1); //mutex lock grants single thread exclusive access to shared resource, ensures thread safety
  //updates global hash table by corresponding cluster index
...
//Initializes multithreaded process for hashing
void global_hashing(){
  if(curr_clusters.size() == 0) return;
  global_paritions.clear();
  if(!first_iteration_using_index || !first_iteration)
    create_anchors("global", global_anchors); //initialise anchors
...
//data structure to store hashing result for clustering algorithm
struct hash_struct{
  string hash_value;
  int rep; //index of cluster representative
  vector<int> cluster; //vector with indexes representing sequences within cluster
};
int randFnc(int j){
  return random_no()%j;
}
...
//Function to cluster DNA strands locally within a single thread
void local_clustering(vector<vector<int> > &clusters){
  ...
  for(int k=0;k<num_local_iter;k++){
    create_anchors("local" , anchors); //initialises local anchor set for hashing on cluster representatives
    
    //Cluster rep is either carried over or renewed by random
    ...
    //Sorting-based clustering to bring similar elements closer: compare adjacent elements for efficiency
    ...
    while(i<n-1){
      ...
      while(j < n){
        ...
        //By paper (hamming_dist <= theta_low),
        //OR (hamming_dist <= theta_high AND edit_dist <= r),
        //for efficiency since hamming_dist is cheaper than edit_dist
        if(hamming_dist<=theta_low){
          if(sorting_or_pairwise){ //merge
            hash_values[i].cluster.reserve(hash_values[i].cluster.size() + hash_values[j].cluster.size()); //vector.reserve() increases vector capacity
...
//Function for local clustering processing on single thread
void *thread_task_assigner(void *args){
  pair<int,int> *info = static_cast<pair<int,int> *>(args);
  int start_index = (*info).first , length = (*info).second;
  for(int i = start_index , no = 0; no < length ; ++no , ++i){
    assert(i < temp_clusters.size());
    local_clustering(temp_clusters[perm[i]]); //local clustering on assigned clusters
  }
  pthread_mutex_lock(&mutex1);
  //aggregate local results in curr_clusters data structure
  for(int i = start_index , no =0; no < length ; ++no,++i){
...
//Partitions global clusters and intialises multithreading
void cluster_paritions(){
  ...
  shuffle(perm.begin(), perm.end(), std::mt19937{std::random_device{}()}); //random shuffle for load balancing of clusters
  temp_clusters.resize(global_paritions.size()); 
  int id = 0;

  //Cluster partitioning using temp_clusters matrix (approximately equal parts)
...
//Function for global clustering, iteratively clustering until clusters
//either stabilise or meet convergence criteria
vector<vector<int> > clustering(const vector<vector<int> > &clusters){
  if(clusters.size() == 0){
    return clusters;
  }
  curr_clusters = clusters;
  float epsilon=4.0/65000; //Clustering accuracy approximately (1 - epsilon) accurate
  int my_count=0;
  ...
  for(i = 0;i < num_global_iter; ++i){
    ...
    //cluster convergence criteria: minute change (curr_val < epsilon) of clusters for 3 iterations
    if(curr_val<epsilon){
      my_count++;
      if(my_count==3){
        break;
...
//Single-thread cleaning process
void *cluster_cleaner(void *args){
  ...
  int start_index = (*info).first , length = (*info).second;
   ...
   for(int i = start_index , no = 0; no < length ; ++no, ++i){
     ...
     //Pushes excessively large cluster to separate data structure
     ...
     //Clusters below size threshold are stored under outliers structure pers_wrongEle
    ...
    for(auto target_strand : curr_clusters[i]){
      ...
      for(auto other_strand : curr_clusters[i]){
        ...
        if(dist > fp_strand_edit_dis_threshold) //count no. of edit_dist of given element beyond threshold (dissimilar)
          ++wrongNum;
      }
      if(wrongNum > fp_strand_fraction_threshold*curr_clusters[i].size()){ //Outlier if element is frequently dissimilar
     ...
     //Treated as outlier if below cluster size threshold
...
//Multithreaded cluster cleaning
//Clears previous temp data clean_temp_clusters and wrong_Ele
//Returns post-cleaning temp data
...
//Attempts to recluster outliers into existing or new clusters
vector<vector<int> > recluster(const vector<vector<int> > &inp_clusters , const vector<int> &wrongEle){
  ...
  //iteratively compare each outlier to each cluster rep,
  //merge if strand meets criteria
  ...
  //clustering over remaining outliers
...
//Single-thread accuracy calculation using predetemined correct cluster set
void  *accuracy_parallel(void *args){
  ...
  for(int i = start_index,no = 0; no < length ; ++i,++no){
    ...
    //iteratively counts clusters that meet correct criteria
    for(auto ele : freq){
...
//Multithreaded accuracy operation
...
int main(){
  ...
  initalise();
  debug.open(getValue("file_locations/debug_info")); //getValue() calls configparser.cpp config data
  ...
  //Reading input, initialise pool - global DNA strands
...
//Initialises variables from config file parsed by configparser.cpp
void initalise(){
  iterable.push_back('A'); //adds new char at the end of vector iterable
